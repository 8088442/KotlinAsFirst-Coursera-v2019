...
fun mergePhoneBooks(mapA: Map<String, String>, mapB: Map<String, String>): Map<String, String> {
    val mutableMap = mapA.toMutableMap()
    for ((key, value) in mapB) {
        val testElement = mutableMap[key]
        if (testElement != null && testElement != value) mutableMap[key] += ", $value"
        else mutableMap[key] = value
    }
    return mutableMap
}
...
fun averageStockPrice(stockPrices: List<Pair<String, Double>>): Map<String, Double> =
    stockPrices.groupBy { it.first }.mapValues { it.value.map { it.second }.average() }
...
fun canBuildFrom(chars: List<Char>, word: String): Boolean =
    word.isEmpty() || chars.isNotEmpty() && word.toLowerCase().toSet().intersect(chars).equals(word.toSet())
